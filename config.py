...
import onet
from models import conv, decoder, OccupancyNetwork
...
def get_generator(model, cfg, device):
    ...
    '''
    method = cfg['method']
    generator = get_generator(model, cfg, device)
    return generator
...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ...
    model_decoder = decoder.DecoderCBatchNorm(
        dim=dim, z_dim=z_dim, c_dim=c_dim,
        **decoder_kwargs
    )
    model_encoder = conv.Resnet18(
        c_dim=c_dim,
        **encoder_kwargs
    )
    ...
    model = OccupancyNetwork.OccupancyNetwork(
        model_decoder, model_encoder, encoder_latent, p0_z, device=device
    )
    return model
