...
class VoxelDecoder(nn.Module):
    def __init__(self, dim=3, z_dim=128, c_dim=128, hidden_size=128):
        ...
        self.convtrp_0 = nn.ConvTranspose3d(256, 128, 3, stride=2,
                                            padding=1, output_padding=1) # output dim (batch_norm,128,)
    ...
    def forward(self, p, z, c, **kwargs):
        ...
        if self.z_dim != 0:
            ...
            net = c 
        # here we have net (c+z) =256    
        net = self.fc_in(net) # (256*4*4*4)
        net = net.view(batch_size, 256, 4, 4, 4) #(batch_size, 256, 4, 4, 4)
        net = self.convtrp_0(self.actvn(net)) #(batch_size, 128, 8,8,8)
        net = self.convtrp_1(self.actvn(net))
        net = self.convtrp_2(self.actvn(net))#(batch_size, 32, 32,32,32)
        ...
        return out #(batch_size, sequence_length) 
